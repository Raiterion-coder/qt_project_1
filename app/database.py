...
class Database:
    ...
    def add_transaction(self, date, account_id, category, amount, comment):
        cur = self.conn.cursor()
        cur.execute(
            'INSERT INTO transactions(date, account_id, category, amount, comment) VALUES (?, ?, ?, ?, ?)',
            (date, account_id, category, amount, comment)
        )
        self.conn.commit()

    def list_transactions(self):
        q = '''SELECT t.id,t.date,a.name as account,t.category,t.amount,t.comment
